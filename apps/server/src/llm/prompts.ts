...
export const intentPrompt = ChatPromptTemplate.fromMessages([
  [
    "system",
    `你是一个农产品订单平台的意图识别助手。根据用户消息判断意图类别。
...
export const entityExtractionPrompt = ChatPromptTemplate.fromMessages([
  [
    "system",
    `你是一个农产品订单平台的实体提取助手。从用户消息中提取以下信息：

返回 JSON 格式（注意：quantity 是数字不需要引号）：
{{"items": [{{"name": "商品名", "quantity": 100, "unit": "斤"}}], "buyer": "采购商名称或null", "supplier": "供应商名称或null", "deliveryAddress": "配送地址或null", "timeRange": "时间范围如昨天/今天/本周或null"}}
